...
class SQLDatabase(Database):
    ...
    CREATE_MATCHES_TABLE = """
        ...
        Database.FIELD_MATCHES_ID , Database.FIELD_MATCHES_ID, Database.FIELD_MATCHES_ID,
    )
    ...
    INSERT_MATCHES = "INSERT INTO %s (%s, %s, %s, %s, %s) values (%%s, %%s ,%%s , %%s, %%s);" % (
        MATCHES_TABLENAME, Database.FIELD_SONG_ID, Database.FIELD_DURATION, Database.FIELD_REAL_TIME,Database.FIELD_FROM_SONG_TIME ,Database.FIELD_TO_SONG_TIME)
    ...
    def empty(self):
        ...
        with self.cursor() as cur:
            ...
            cur.execute(self.DROP_MATCHES_TABLE)
    ...
    def insert_matches(self, matches):
        ...
        with self.cursor() as cur:
            cur.execute(self.INSERT_MATCHES, matches)
            return cur.lastrowid
    ...
    def return_matches(self, hashes):
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                ...
                for hash, sid, offset in cur:
                    # (sid, db_offset - song_sampled_offset)
                    yield (sid, offset - mapper[hash],offset,mapper[hash])
